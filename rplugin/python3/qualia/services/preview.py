...
path.append(Path(__file__).parent.parent.parent.as_posix())
from qualia.services.utils.preview_utils import InvalidParameters, parse_args, connect_listener  # noqa: E402

if TYPE_CHECKING:
    from qualia.models import ListenerRequest, SourceId


def main() -> None:
    try:
        attach_running_process, node_id_arg, source_id_arg, output_width, output_height = parse_args()
    ...
    if attach_running_process:  # Takes under 0.1s if uneeded imports are lazy loaded
        ...
        if conn:
            preview_node_request: ListenerRequest = ('preview_node', [node_id_arg, source_id_arg, output_width, output_height, 3], {})
    ...
    preview_source_id = cast(SourceId, source_id_arg)
    try:
        print(
            '\n'.join(cast(list[str], preview_node(preview_node_id, preview_source_id, output_width, output_height, _PREVIEW_NEST_LEVEL))))
