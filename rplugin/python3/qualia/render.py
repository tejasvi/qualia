...
def render(root_view, buffer, nvim, db, fold_level):
    # type:(View, Buffer, Nvim, MinimalDb, Optional[int]) -> LastSync
    new_last_sync, new_content_lines = get_buffer_lines_from_view(root_view, db, fold_level)
    ...
    if DEBUG:
        try:
            new_root_view, new_changes = ParseProcess().process_lines(cast(Li, new_content_lines.copy()),
                                                                      root_view.main_id,
                                                                      new_last_sync, db, root_view.transposed)
            assert not new_changes, '\n'.join( map(str, (old_content_lines, new_content_lines, new_changes, new_last_sync)))
            re_last_sync, re_content_lines = get_buffer_lines_from_view(new_root_view, db, fold_level)
            assert new_content_lines == re_content_lines, '\n'.join(new_content_lines + ['<TO>'] + re_content_lines)
        except Exception as exp:
            for _ in range(100):
                new_root_view, new_changes = ParseProcess().process_lines(cast(Li, new_content_lines.copy()), root_view.main_id, new_last_sync, db, root_view.transposed)
                new_root_view, new_changes = ParseProcess().process_lines(cast(Li, new_content_lines.copy()), root_view.main_id, new_last_sync, db, root_view.transposed)
                re_last_sync, re_content_lines = get_buffer_lines_from_view(new_root_view, db, fold_level)
            raise exp
...
def get_buffer_lines_from_view(buffer_view: View, db: MinimalDb,
                               fold_level: Optional[int]) -> tuple[LastSync, Li]:
    transposed = buffer_view.transposed
    last_sync = LastSync(None)
    buffer_lines = cast(Li, [])
    stack: list[tuple[NodeId, View, int, int, bool, int]] = [(buffer_view.main_id, View(cast(NodeId, InvalidNodeId()), buffer_view.source_id, {
    ...
    while stack:
        ...
        content_lines = db.get_node_content_lines(cur_node_id, temporary)
        descendant_ids = db.get_node_descendants(cur_node_id, transposed, True, temporary)
        ...
        if buffer_descendant_context is not None:
            ...
            for descendant_node_id in sorted(descendant_ids, reverse=True) if _SORT_SIBLINGS else reversed(
                ...
                stack.append(
                    (
                        descendant_node_id, View(cur_node_id, buffer_view.source_id, descendant_context, transposed), current_indent_level,
                        cur_nest_level + 1,
