...
from qualia import states, Ledger
...
def sync_buffer(buffer: Buffer, ledger: Ledger) -> View:
    root_id = NodeId(basename(buffer.name))  # .rsplit('.q.md', maxsplit=1)
    create_root_if_new(root_id)
    if root_id in ledger:
        root_view, changes = Process().process_lines(list(buffer), root_id, ledger)
        sync_with_db(root_view, changes, ledger)
    else:
        root_view = View(root_id, get_key_val(root_id, states.cursors.views) or {})
    return root_view


def sync_with_db(root_view: Optional[View], changes: ProcessState, ledger: Ledger):
    ...
    sync_content(changes.changed_content_map, cursors.content, cursors.unsynced_content, ledger)
    sync_children(changes.changed_children_map, cursors.children, cursors.unsynced_children, ledger)
...
def sync_children(changed_children_map: dict[NodeId, OrderedSet], children_cur: lmdb.Cursor,
                  unsynced_children_cur: lmdb.Cursor, ledger: Ledger) -> None:
...
def sync_content(changed_content_map: dict[NodeId, list[str]], content_cur: lmdb.Cursor,
                 unsynced_content_cur: lmdb.Cursor, ledger: Ledger) -> None:
